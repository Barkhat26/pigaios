...
from argsgenerators.utils import is_source_file, path_endswith
from argsgenerators.BaseArgsGenerator import BaseArgsGenerator
...
class SimpleArgsGenerator(BaseArgsGenerator):
    ...
    def generate(self):
        ...
        include_to_include_dir = pide.get_project_include_dirs(files_to_includes)

        print('[+] Generating files with -I arguments...')
        file_to_args = {}
        for f in files_without_parent:
            include_dirs = set()
            for include in files_to_includes[f]:
                if include in include_to_include_dir:
                    include_dir = include_to_include_dir[include]
    ...
    def _get_files_without_parent(files_to_includes):
        ...
        for filename, includes in files_to_includes.items():
            g.add_node(filename)
            for include in includes:
                g.add_node(include)
                g.add_edge(filename, include)
...
class ProjectIncludesExtractor:
    ...
    def _extract_included_file(string):
        ...
        Returns:
            ...
            if string parameter doesn't contain #include statement
            then returns None
        """
        ...
        if result:
            ...
            # but for convenient further processing we replace backslashes with slashes
            if '\\' in included_file: 
...
class ProjectIncludeDirsExtractor:
    ...
    def get_project_include_dirs(self, files_to_includes):
        ...
        Returns:
            include_to_include_dir (dict of str:str): includes with directories where
                they can be found
        """
    
        includes = self._extract_includes(files_to_includes)
        include_to_include_dir = self._traverse_dirs(includes)
    
        return include_to_include_dir
    ...
    def _traverse_dirs(self, includes):
        ...
        Returns:
            include_to_include_dir (dict of str:str): include and directory
                path where the include can be found
    
        """
    
        include_to_include_dir = {}
        ...
        for include in includes:
            ...
            if filepath:
                ...
                include_to_include_dir[include] = include_dir
