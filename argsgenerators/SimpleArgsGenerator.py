...
class SimpleArgsGenerator(BaseArgsGenerator):
  ...
  def generate(self):
    ...
    for f in files_without_parent:
      self.file_to_args[f] = ['-I{}'.format(pi) for pi in include_dirs]

    self.file_to_args = OrderedDict(sorted(self.file_to_args.items(), key=lambda x: x[0]))
    return self.file_to_args

  def _get_files_without_parent(self, files_to_includes):
...
class ProjectIncludesExtractor:
  ...
  def _extract_included_file(self, string):
...
class ProjectIncludeDirsExtractor:
  ...
  def _find_in_project_dir(self, include, project_files):
  ...
  def _extract_includes(self, data):
